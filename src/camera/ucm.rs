//! Unified Camera Model (UCM) Implementation
//!
//! This module implements the Unified Camera Model (UCM), which is particularly useful
//! for wide-angle and fisheye cameras. The model uses a single sphere projection to handle
...
//! # References
//!
//! The Unified Camera Model is based on:
//! "A Unifying Theory for Central Panoramic Systems and Practical Applications" by Christopher Geyer and Kostas Daniilidis.
...
/// Implements the Unified Camera Model (UCM) for wide-angle/fisheye lenses.
///
/// The UCM model is designed for cameras with significant distortion,
...
/// and one distortion parameter: `alpha`.
/// `alpha` controls the projection onto a unit sphere.
///
/// # Fields
///
...
/// *   `alpha`: `f64` - The distortion parameter, controlling the sphere projection.
///     It must be a finite number.
///
/// # References
///
/// *   Geyer, C., & Daniilidis, K. (2000). A unifying theory for central panoramic systems
///     and practical applications. In *European conference on computer vision* (pp. 445-461). Springer.
///
/// # Examples
///
/// ```rust
/// use nalgebra::DVector;
/// use vision_toolkit_rs::camera::ucm::UcmModel;
...
/// let mut ucm_model = UcmModel::new(&params).unwrap();
/// ucm_model.resolution = Resolution { width: 752, height: 480 };
///
/// println!("Created UCM model: {:?}", ucm_model);
/// assert_eq!(ucm_model.intrinsics.fx, 1313.83);
/// assert_eq!(ucm_model.alpha, 1.01674);
/// ```
...
pub struct UcmModel {
    ...
    /// Distortion parameter, controlling the sphere projection.
    /// Must be a finite `f64` value.
    pub alpha: f64,
}

impl UcmModel {
    /// Creates a new [`UcmModel`] from a DVector of parameters.
    ///
    ...
    ///   5.  `alpha`: The distortion parameter.
    ///
    /// # Return Value
    ///
    ...
    ///     1.01674  // alpha
    /// ]);
    /// let mut model = UcmModel::new(&params_vec).unwrap();
    ...
    /// Checks the geometric condition for a valid projection in the UCM model.
    ///
    /// This helper function determines if a 3D point can be validly projected
    /// based on its z-coordinate (`z`), its distance from the origin (`d`),
    /// and the model's `alpha` parameter.
    ///
    /// # Arguments
    ///
    /// * `z`: `f64` - The Z-coordinate of the 3D point in camera space.
    /// * `d`: `f64` - The Euclidean distance of the 3D point from the camera origin.
    /// * `alpha`: `f64` - The distortion parameter.
    ///
    /// # Return Value
    ///
    /// Returns `true` if the point satisfies the projection condition, `false` otherwise.
    pub fn check_proj_condition(z: f64, d: f64, alpha: f64) -> bool {
        let w = if alpha <= 0.5 {
            alpha / (1.0 - alpha)
        } else {
            (1.0 - alpha) / alpha
    ...
    /// Checks the geometric condition for a valid unprojection in the UCM model.
    ///
    ...
    /// The condition depends on the model's `alpha` parameter.
    ///
    /// # Arguments
    ///
    ...
    fn check_unproj_condition(r_squared: f64, alpha: f64) -> bool {
        if alpha > 0.5 {
            let gamma = 1.0 - alpha;
...
/// Provides a debug string representation for [`UcmModel`].
impl fmt::Debug for UcmModel {
...
impl CameraModel for UcmModel {
    /// Projects a 3D point from camera coordinates to 2D image coordinates.
    ///
    /// This method applies the UCM projection equations. It first checks
    /// if the point is projectable using the UCM geometric constraints.
    ...
    ///   according to the UCM model's geometric constraints.
    fn project(&self, point_3d: &Vector3<f64>) -> Result<Vector2<f64>, CameraModelError> {
        ...
        if denom < PRECISION || !Self::check_proj_condition(z, d, self.alpha) {
    ...
    ///
    /// This method applies the inverse UCM model equations to convert
    ...
    fn unproject(&self, point_2d: &Vector2<f64>) -> Result<Vector3<f64>, CameraModelError> {
        ...
        let gamma = 1.0 - alpha;
        let xi = alpha / gamma;
        let mx = (u - cx) / fx * gamma;
        let my = (v - cy) / fy * gamma;
        ...
        if denom < PRECISION || !Self::check_unproj_condition(r_squared, alpha) {
            return Err(CameraModelError::PointIsOutSideImage);
        }
    ...
    /// Loads [`UcmModel`] parameters from a YAML file.
    ///
    /// The YAML file is expected to follow a structure where camera parameters are nested
    /// under `cam0`. The intrinsic parameters (`fx`, `fy`, `cx`, `cy`) and the UCM
    /// specific distortion parameter (`alpha`) are typically grouped
    /// together in an `intrinsics` array in the YAML file: `[fx, fy, cx, cy, alpha]`.
    ...
    /// of [`UcmModel`] populated with parameters from the file.
    ///
    /// # Errors
    ///
    ...
    /// Saves the [`UcmModel`] parameters to a YAML file.
    ///
    /// The parameters are saved under the `cam0` key. The `intrinsics` YAML array
    /// will contain `fx, fy, cx, cy, alpha` in that order.
    ///
    /// # Arguments
    ///
    ...
    /// Validates the parameters of the [`UcmModel`].
    ///
    ...
    /// * [`CameraModelError::InvalidParams`]: If `alpha` is not a finite number.
    ...
    /// Returns the distortion parameters of the UCM model.
    ///
    /// The parameter is returned as a vector containing `[alpha]`.
    ///
    /// # Return Value
    ///
    /// A `Vec<f64>` containing the distortion parameter: `[alpha]`.
...
mod tests {
    ...
    /// Helper function to create a sample [`UcmModel`] instance for testing.
    /// This model is based on parameters from "samples/ucm.yaml".
    fn get_sample_model() -> UcmModel {
        UcmModel {
            intrinsics: Intrinsics {
                fx: 1313.83,
                fy: 1313.27,
    ...
    fn test_ucm_load_from_yaml() {
        ...
        let model = UcmModel::load_from_yaml(path).unwrap();

        assert_eq!(model.intrinsics.fx, 1313.83);
        assert_eq!(model.intrinsics.fy, 1313.27);
        assert_eq!(model.intrinsics.cx, 960.471);
        assert_eq!(model.intrinsics.cy, 546.981);
        assert_eq!(model.alpha, 1.01674);
        assert_eq!(model.resolution.width, 752);
        assert_eq!(model.resolution.height, 480);
    }

    /// Tests saving [`UcmModel`] parameters to a YAML file and then reloading them.
    #[test]
    fn test_ucm_save_to_yaml() {
        use std::fs;
        ...
        let output_path = "output/ucm_saved.yaml";

        // Load the camera model from the original YAML
        let model = UcmModel::load_from_yaml(input_path).unwrap();

        // Save the model to the output path
        model.save_to_yaml(output_path).unwrap();

        // Load the model from the saved file
        let saved_model = UcmModel::load_from_yaml(output_path).unwrap();
    ...
    /// Tests the consistency of projection and unprojection for the [`UcmModel`].
    #[test]
    fn test_ucm_project_unproject() {
        ...
        // Use a more relaxed epsilon for UCM due to numerical precision in the complex equations
    ...
    #[test]
    fn test_ucm_project_unproject_center() {
    ...
    fn test_project_point_at_center() {
        ...
        // For UCM, if `denom` becomes too small or `check_proj_condition` fails,
