...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let total_time = all_metrics
        ...
        .map(|m| m.optimization_time_ms)
        .sum::<f64>();
    let total_improvements: f64 = all_metrics
        .iter()
        .map(|m| m.initial_reprojection_error - m.final_reprojection_error)
        .sum();

    println!("📊 Average Reprojection Error: {:.6} pixels", avg_error);
    println!("📊 Average Optimization Time: {:.2} ms", avg_time);
    println!(
        "📊 Total Optimization Improvement: {:.6} pixels",
        total_improvements
    );
...
fn convert_kb_to_double_sphere(
    ...
    // Non-linear optimization with verbose output to get more info
...
fn export_rust_results(
    ...
    report.push_str("=== FISHEYE CAMERA MODEL CONVERSION RESULTS ===\n\n");
    ...
    report.push_str("Source Model: Kannala-Brandt\n\n");
    ...
    // Write to file
    std::fs::write("fisheye_conversion_results.txt", report)?;
    println!("📄 Results exported to: fisheye_conversion_results.txt");
    info!("📄 Results exported to: fisheye_conversion_results.txt");
